  ...
  { 'folke/which-key.nvim',  opts = {} },
  {
    ...
    opts = {
      ...
      on_attach = function(bufnr)
        ...
        map('n', '<leader>gbr', gs.reset_buffer, { desc = '[G]it [Buffer] [R]eset' })
...
-- Make line numbers with relative number
vim.wo.number = true
...
-- Remap for more intuitive line navigation
vim.keymap.set('n', 'H', '^')
vim.keymap.set('n', 'L', '$')
...
-- Disable neovim python provider
...
vim.defer_fn(function()
  require('nvim-treesitter.configs').setup {
    ...
    ensure_installed = { 'c', 'cpp', 'go', 'lua', 'python', 'rust', 'tsx', 'typescript', 'vimdoc', 'vim', 'prisma', 'bash' },
  ...
  nmap('gd', function()
    vim.lsp.buf.definition {
      on_list = function(options)
        ...
        if #items > 1 then
          items = require('utils').filterLspBufItems(items, function(value)
            return not require('utils').isZodTypeInfer(value)
          end)
        end
